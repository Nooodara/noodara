...
describe('createServerServices factory (D-01, D-17)', () => {
  ...
  // listConnectingServerIds); Plan 04-08 (Task 1) grew it further to nine (getServer,
  // listServers); Plan 04-10 (Task 2) grew it to ten (listActivity); Plan 05-05 (Task 1) grows it
  // to eleven (readLatestDiscovery) — the exhaustive-membership assertion itself now lives in
  // read-servers.test.ts's own "ServerServices facade exposes reads" suite.
  it('returns the five phase-3 service keys plus the phase-4/5 recovery and read members', async () => {
    fixture = await startServiceFixture();
