...
| T1 | 05-21 | 15 | QA-05 | T-5-97 / T-5-98 | No canary secret in rendered HTML, console, storage, URL, history, bodies, SSE, logs, activity | integration + E2E | `pnpm security:scan-leaks` | Partial — UI-output scan is this task | ✅ green (initially hung on ~half its runs, misattributed to host memory pressure; the real cause was a test-only hang in `response.text()` on navigation-abandoned requests, fixed `c41701a`, verified 12/12 — see 05-21-SUMMARY.md and deferred-items.md) |
...
| T3 | 05-21 | 15 | UI-01, UI-02 | T-5-101 | Design-system conformance in both themes + the contrast decision | **blocking human checkpoint** | manual `noodara-ux-review` audit → `docs/ui-review-05.md` | n/a | ✅ resolved — user approved closing the phase (2026-09-20); this is **not** a statement of visual/design approval (verbatim: "No me gusta la UI pero la vamos a ir mejorando con el tiempo"). The contrast gap is routed to gap closure, not accepted as-is. See 05-21-SUMMARY.md's checkpoint section. |
...
**Approval:** Plan 05-21 Task 2 confirms every row above (2026-09-20) — every automated-verify task row is green. 05-21 T1 (`@canary`) initially hung on about half its runs; that was first misattributed to host memory pressure and was in fact a test-only defect (response bodies read from an event that never settles for navigation-abandoned requests), fixed in `c41701a` and verified 12/12 -- see deferred-items.md.

**Task 3 checkpoint resolution (user, Pablo Gutierrez, 2026-09-20):** the user walked the running app themselves through a temporary public tunnel to a throwaway instance (ephemeral secrets, torn down afterwards). Live updates (SSE-driven list insertion, discovery progress) could not be observed in that session because the tunnel buffers Server-Sent Events — they remain covered by E2E only, not by human eyes. Verdict, verbatim: **"No me gusta la UI pero la vamos a ir mejorando con el tiempo. Por el momento le doy approve."** This means: approved to close the phase. It does **not** mean the visual design was found satisfactory — the user explicitly said they dislike the current UI and expects iterative improvement. No specific visual defect was named. The light-mode status-pill contrast gap, the missing floating-elevation shadow, and `RowMenu`'s missing `aria-expanded` are **not accepted as-is** — all three are routed to a follow-up gap-closure plan, not held open against this phase and not silently accepted. The phase is closed on that basis; nothing in this document should be read as a human PASS on visual design. See `docs/ui-review-05.md`'s own "Human walkthrough" section and `05-21-SUMMARY.md` for the full record.
