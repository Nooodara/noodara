...
- [ ] **AUTH-02**: El admin inicia sesión con email y password (argon2id vía Better Auth) y la sesión persiste entre recargas del navegador.
- [ ] **AUTH-03**: El admin cierra sesión desde cualquier pantalla y la sesión queda invalidada en el servidor.
- [ ] **AUTH-04**: El login está limitado por tasa por IP y por cuenta; los intentos fallidos se registran en el activity log sin incluir el password.
...
- [ ] **SEC-01**: Las credenciales SSH se cifran at-rest con AES-256-GCM, nonce único por operación y metadato de versión de clave por fila, listo para rotación.
...
- [ ] **QA-01**: Cada PR corre lint, typecheck, unit tests, integration ligera, gitleaks y `pnpm audit`; el merge se bloquea si algo falla.
- [ ] **QA-02**: `packages/domain` mantiene ≥95% statement y ≥95% branch en validadores y state machines, verificado por umbral en CI.
