...
- [Phase 06 P08 post-execution fix]: noodara_ensure_docker's docker-daemon-unavailable gate (exit 22) fires on "docker binary present && docker version fails" alone -- it does not attempt to distinguish "daemon stopped" from any other reason docker version might fail (permissions, corrupted install, etc.). Any binary-present-but-broken state stops immediately, names it, and never touches apt/gpg/systemctl -- a single zero-side-effect check rather than a diagnostic tree.
- [Phase 06 P09 post-execution fix, Finding A]: noodara_check_port skips the busy-port check entirely on an existing installation (NOODARA_INSTALL_DIR/.env exists) rather than checking against the port recorded in .env -- the recorded port being busy is always expected (it is this installation's own running container) so there is nothing to verify; an operator-supplied NOODARA_PORT override that disagrees with .env's recorded value gets exactly one warning (from noodara_check_port itself) naming both values, never repeated by noodara_main's own later port read.
- [Phase 06 P09 post-execution fix, Finding B]: noodara_resolve_installed_public_url/noodara_main read NOODARA_PORT and NOODARA_PUBLIC_URL directly from .env on every re-run (falling back to full network resolution only when the key is genuinely absent from .env) instead of re-resolving unconditionally; noodara_check_ufw and noodara_print_summary now take the resolved port as an explicit parameter (default: noodara_resolve_port, unchanged for every existing caller) so the ufw advisory can never name a different port than the summary.
- [Phase 06 P09 post-execution fix, Finding C]: interpreted D-09's "si ya está en esa versión, no cambia nada y solo verifica salud" literally, not as a silence to fill in with judgment -- a same-version re-run with nothing missing from .env skips docker compose pull AND docker compose up -d entirely (not just the .env backup/merge), running only the health check and summary.
- [Phase 06 P09 post-execution fix, Finding D]: noodara_probe_admin_exists reuses the existing POST /api/setup route (via docker compose exec -T api node -e, no apps/control-plane code change) as the primary "does an admin exist" signal, since it is checked before the submitted token/email/password and writes no activity event or lockout state on the failure path; a "missing"/"unknown" probe result (including any case where the exec probe itself cannot run) falls back to the original D-13 log-reading heuristic, which is this fix's one documented, honest limitation -- no apps/control-plane follow-up is proposed since the existing route is already sufficient.
- [Phase 06 P09 post-execution fix, Finding E]: a same-version no-op *candidate* (Finding C: version matches .env, every merge key present) is only treated as a true no-op after a single, immediate noodara_stack_healthy_now read (one noodara_service_health call per service, no sleep, no polling loop) confirms api/web are genuinely healthy -- reading D-09's "no cambia nada y solo verifica salud" as a real health check with a real consequence, not a label applied unconditionally from .env's own state. When the candidate's stack is not healthy (a half-finished first install whose pull/up never succeeded, or a manually stopped stack), this is a repair: still no .env backup/merge (nothing in .env needs to change), but pull + docker compose up -d + the real bounded noodara_wait_for_health run exactly as a normal re-run would; noodara_wait_for_health's D-12 rollback hint takes an optional "repair" context so it never claims an upgrade happened when the version never changed.
- [Phase 06 P14 post-execution fix, Finding 3]: kept D-09's same-version/healthy-stack no-op unchanged (a re-run still never runs docker compose up in that case), and instead made the operator-facing advice true: applying an edited /opt/noodara/.env is `docker compose -f <install dir>/docker-compose.yml up -d`, never "edit .env and re-run the installer" -- docs/install.md's "Supported variables" and "Plain HTTP warning" TLS procedure, plus install.sh's three .env-mismatch/resolved-URL messages (noodara_check_port, noodara_resolve_public_url, noodara_resolve_installed_public_url), were rewritten to say so; installer-driven configuration-drift auto-apply (docker compose config --hash vs. the container's own config-hash label) was deliberately left as a follow-up idea, not implemented.
