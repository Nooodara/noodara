...
| T-4-02 | Elevation of Privilege | live SSE stream after logout/revocation | mitigate | heartbeat re-resolves session every interval, ends stream when gone | **open** |
...
| T-4-10 | Information Disclosure | worker `failed`/`warn` logs | mitigate | claimed: never `job.data`/credential, "err serializer is pino's, already redaction-configured" | **open** |
...
| T-4-32 | Denial of Service | shutdown hanging on active job | mitigate | claimed: `Promise.race` against D-14 budget "guarantees termination" | **open** |
...
| T-4-38 | Information Disclosure | publish-failure logs | mitigate | claimed: "no Redis URL, host or port ever gets string-interpolated into a log line" | **open** |
...
- [ ] `threats_open: 0` confirmed — **4 open** (T-4-02, T-4-10, T-4-32, T-4-38)
- [ ] `status: verified` set in frontmatter — **status: open**, not verified

**Approval:** pending — phase must not ship until T-4-02, T-4-10, T-4-32 and T-4-38 are remediated or explicitly re-dispositioned as accepted risk with a logged rationale, and UF-01 (TOFU bypass) is triaged.
