...
| 1-16-01 | 16 | 9 | INST-06 | T-1-54 | Real-boot child-process smoke test reproduces then closes the BLOCKER: `start` fail-fast, `start` real boot, package-scoped `dev`, clean-tree turbo-driven root `dev` | integration | `pnpm exec vitest run --config vitest.integration.config.ts tests/integration/boot/boot-command.test.ts` | ✅ | ✅ green |
| 1-16-02 | 16 | 9 | INST-06 | T-1-55 / T-1-56 | `packages/domain` builds to `dist` with `.d.ts` declarations; `exports` resolves the same five entrypoints under plain Node with zero `tsx` involvement | build | `pnpm --filter @noodara/domain build && node -e "require('@noodara/domain')"` (ESM: `pnpm build && node --input-type=module -e "import '@noodara/domain'"`) | ✅ | ✅ green |
| 1-16-03 | 16 | 9 | INST-06 | T-1-57 | `apps/control-plane`'s `dev` runs through `tsx watch`; `start` runs the built `dist/server.js` under plain Node with no loader; `dist` is self-contained (migrations copied, test files excluded) | integration | `pnpm build && env -i "$(command -v node)" apps/control-plane/dist/server.js` (fails fast on empty env, per `1-16-01`'s first case) | ✅ | ✅ green |
| 1-17-01 | 17 | 10 | QA-01 | T-1-58 / T-1-60 | `boot-smoke` CI job builds the artifact and runs the real-boot smoke suite on every pull request, including the clean-tree turbo-driven root `pnpm dev` case; no `continue-on-error` introduced | static/CI | `actionlint .github/workflows/ci.yml && pnpm build && pnpm test:boot` | ✅ | ✅ green |
| 1-17-02 | 17 | 10 | QA-01, INST-06 | T-1-59 | ADR 0003 records the runtime/module-resolution contract as Accepted; CLAUDE.md §3.2 and this validation map both name the real boot commands the phase goal is measured against | docs/static | `test -f docs/adr/0003-runtime-entrypoints-and-module-resolution.md && grep -q "^## Status" docs/adr/0003-runtime-entrypoints-and-module-resolution.md && grep -q "pnpm start" CLAUDE.md` | ✅ | ✅ green |

*Status: ⬜ pending · ✅ green · ❌ red · ⚠️ flaky*

*38/38 tasks green. Zero `.skip`/`.todo` in `tests/`, `packages/`, `apps/` (`grep -rc "\.skip\|\.todo" tests/ packages/ apps/ \| grep -v ':0' \| wc -l` → `0`).*

**Note — the gap this closes:** the phase's original 33-row map (through Plan 01-15) contained no row that exercised a real `pnpm dev` or `node dist/server.js` invocation; every "boot" proof went through Vitest's own module transform or `tsx`, which is precisely how `01-VERIFICATION.md`'s BLOCKER (a `pnpm dev` / `dist/server.js` boot failure) survived a fully green CI pipeline through the end of the phase. Rows `1-16-01`..`1-16-03` and `1-17-01`..`1-17-02` above are the closure: they are the first rows in this table whose Automated Command spawns the actual documented command as a real child process. Future phases should add at least one boot-path row (a real spawned command, not a test-harness helper) before declaring any new entrypoint or process-boundary task done.
